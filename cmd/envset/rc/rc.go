...
func GetCommand(cnf *config.Config) *cli.Command {
	return &cli.Command{
		...
		Flags: []cli.Flag{
			&cli.BoolFlag{
				Name:  "print",
				Usage: "only print the contents to stdout, don't write file",
			},
			&cli.StringFlag{
				Name:  "filename",
			...
			&cli.BoolFlag{
				Name:  "overwrite",
				Usage: "set true to prevent overwrite metadata file",
				Value: true,
			},
			&cli.BoolFlag{
				Name:  "values",
				Usage: "add flag to show values in the output",
			},
			&cli.BoolFlag{
				Name:  "globals",
				Usage: "include global section",
				Value: false,
			},
			&cli.StringFlag{
				Name:    "secret",
				Usage:   "`password` used to encode hash values",
